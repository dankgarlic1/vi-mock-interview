...
const Dashboard = () => {
  ...
  const sendMessage = async () => {
    ...
    const input_with_context = ` U are an interview coach expert  this is the details of  the candidate ${JSON.stringify(response)} help them with any interview queries the query of the user is ${input};`
    try {
    ...
    <div className="flex flex-col h-full rounded-xl w-full justify-center items-center  bg-background dark:bg-[#202434] font-sans ">
      ...
      <header className="flex items-center justify-between px-6 py-4 border-b  dark:border-[#293040] border-[#E9ECF1]">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h1 className="text-lg font-semibold">AI Interview Coach</h1>
            <p className="text-sm text-muted-foreground">
              Helping you prepare for technical and behavioral interviews
            </p>
          </div>
        </div>
        <Button
      ...
      <ScrollArea className="flex-1 p-4 overflow-y-auto">
        <div className="space-y-4">
          ...
          {isStreaming && (
            <div className="text-muted-foreground">Interview Coach is typing...</div>
          )}
        </div>
      </ScrollArea>
      ...
      <div className="p-2 border shadow-xl dark:border-[#293040] rounded-xl bg-[#F3F4F6] dark:bg-[#212A39] border-[#E9ECF1] mb-4">
        <div className="flex gap-2">
          <Input
            placeholder="Type your interview question or ask for practice..."
            value={input}

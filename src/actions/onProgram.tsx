    ...
    for (const [index, row] of data.entries()) {
      try {
        ...
        try {
          ...
          // Generate embedding
          const textToEmbed = `${validatedData.role} interview question: ${validatedData.question} - ${validatedData.answer || ""}`;
...
export async function importDataFromExcel() {
  try {
    const interviewFilePath = path.join(process.cwd(), 'src', 'InterviewQuestions.csv');
    
    console.log('Attempting to import from:', {
      interviewPath: interviewFilePath
    });

    const interviewResult = await bulkImportInterviewResources(interviewFilePath);

...
export class OpenAIAssistant {
  ...
  private getNextFallbackQuestion(): string {
    ...
    this.fallbackQuestionIndex =
      (this.fallbackQuestionIndex + 1) % this.fallbackQuestions.length;
    return question;
  }
      ...
      const completion = await this.client.chat.completions.create({
        messages: [
          ...
          },
          {
            role: 'system',
            content:
              "Follow the interview flow: Start by greeting the intern and introducing yourself as a representative from Virtual Internships. Then ask a general opening question like 'Tell me about yourself'. Keep it under 50 words and be warm and professional.",
          },
        ],
  ...
  async getResponse(
    ...
    try {
      const dynamicContext =
        interviewContexts.length > 0
          ? `Here are relevant interview questions:\n${interviewContexts.join('\n\n')}`
          : '';
...
- Be natural and conversational`,
      ...
      const fallback = this.getNextFallbackQuestion();
  ...
  async getAdditionalContext(userMessage: string): Promise<{
    ...
    try {
      ...
      const resourceLines = responseText
        .split('\n')
        .filter(
          (line) =>
      ...
      const questionsText =
        questionStartIndex > -1
          ? responseText.slice(questionStartIndex)
          : responseText;
    ...
    this.conversationHistory = [
      {
        role: 'system',
        content:
          'You are a technical interview coach helping candidates prepare for software engineering interviews.',
      },
    ];

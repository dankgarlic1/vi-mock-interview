...
export function generateCandidateDetails(candidate: any): string {
  return `
    Meet **${candidate.name || "the candidate"}**, a ${candidate.age || "N/A"}-year-old **${
    candidate.currentRole || "N/A"
  }** with **${candidate.yearsOfExperience || "N/A"}** years of experience, targeting **${
    candidate.targetRole || "N/A"
  }** roles. 
    
    A citizen of **${candidate.nationality || "N/A"}**, ${candidate.name} has skills in **${
    candidate.skills || "N/A"
  }** and is interested in opportunities at **${candidate.preferredCompanies || "N/A"}**. 
    
    Career Goals: **${candidate.careerGoals || "N/A"}**
    
    Contact: **${candidate.email || "N/A"}** | **${candidate.phone || "N/A"}**. ${
    candidate.filledProfile ? "Profile completed." : "Profile pending."
  } ${
    candidate.interviewPreferences && candidate.interviewPreferences !== "None"
      ? `Interview preferences: **${candidate.interviewPreferences}**.`
      : ""
  }
    Updated on **${new Date(candidate.updatedAt).toLocaleDateString()}**.
  `;
}
